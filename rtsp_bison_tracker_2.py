...
class HLSManager:
    ...
    def __init__(self, width: int, height: int, fps: float, segment_time=2, list_size=6, delete_old=True):
        ...
        # small buffer to avoid memory growth
...
class StreamManager:
    ...
    def start_stream(self):
        ...
        self.hls = HLSManager(width, height, fps,
                              HLS_SEGMENT_TIME, HLS_LIST_SIZE, HLS_DELETE_OLD)
    ...
    def _process_frame_with_model(self, frame, frame_count):
        try:
            results = self.model.track(
                source=frame,
                tracker=TRACKER_CFG if os.path.exists(
                    TRACKER_CFG) else "bytetrack.yaml",
                conf=MIN_CONFIDENCE,
                persist=True,
                verbose=False
            )[0]
            ...
            if boxes is not None:
                ...
                ids_tensor = boxes.id
                id_list = ids_tensor.tolist() if ids_tensor is not None else [
                    None]*len(cls_list)
                ...
                for (x1, y1, x2, y2), tid, cls, conf in zip(coords, id_list, cls_list, conf_list):
                    ...
                    color_intensity = int(
                        255 * max(0.0, min(1.0, float(conf))))
            ...
            self.stats['max_bison_in_frame'] = max(
                self.stats['max_bison_in_frame'], bison_count)
            if frame_confidences:
                self.stats['avg_confidence'] = sum(
                    frame_confidences) / len(frame_confidences)
...
class StreamingHandler(BaseHTTPRequestHandler):
    ...
    def serve_hls_playlist(self):
        ...
        if not hls or not hls.enabled:
            ...
            self.end_headers()
            self.wfile.write(
                b"HLS not available (ffmpeg not found or failed to start).")
            return
    ...
    def generate_html_player(self):
        ...
        hls_available = bool(
            self.stream_manager.hls and self.stream_manager.hls.enabled)
...
def get_user_input():
    ...
    print("=" * 50)
    rtsp_url = input(
        f"Enter RTSP URL (default: {DEFAULT_RTSP_URL}): ").strip() or DEFAULT_RTSP_URL
